# AgentsHomeOffice Tauri Desktop Shell

这个目录用于把 `AgentsHomeOffice` 包成桌面应用（透明窗口），并在启动时自动拉起后端进程。
...
```bash
cd /Users/wangzhaohan/Documents/GitHub/AgentsHomeOffice
uv venv .venv
...
```bash
cd /Users/wangzhaohan/Documents/GitHub/AgentsHomeOffice/desktop-pet
npm install
npm run dev
```
